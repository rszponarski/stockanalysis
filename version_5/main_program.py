...
def input_data():
    ...
    stock_market_data(selected_index, start_date, end_date, interval, volume_checkbox_var.get(), max_min_checkbox_var.get())
...
def toggle_date_range_button():
    if preset_var.get() == 4:
        ...
        hide_date_fields()
        download_button.grid(row=10, column=0, columnspan=2)  # Move download button to row 10


def show_date_fields():
    start_date_label.grid(row=3, column=2)
    start_date_entry.grid(row=3, column=3)
...
root = Tk()
root.title("Stock Price Data Downloader")

# Section for index selection
Label(root, text="Select index:", font=("Arial", 11, "bold"), fg="#003366").grid(row=0, column=0, columnspan=2)
index_var = StringVar(value='WIG20')
index_combobox = ttk.Combobox(root, textvariable=index_var, values=['WIG20', 'mWIG40', 'sWIG80', 'other'])
index_combobox.grid(row=0, column=2, columnspan=2)

# Section for preset date range options
Label(root, text="Select chart range:", font=("Arial", 11, "bold"), fg="#003366").grid(row=1, column=0, columnspan=4)
preset_var = IntVar(value=2)
Radiobutton(root, text="Last Week", variable=preset_var, value=1, command=toggle_date_range_button).grid(row=2, column=0)
Radiobutton(root, text="Last Month", variable=preset_var, value=2, command=toggle_date_range_button).grid(row=2, column=1)
Radiobutton(root, text="Last Quarter", variable=preset_var, value=3, command=toggle_date_range_button).grid(row=2, column=2)
Radiobutton(root, text="Select Specific Date Range", variable=preset_var, value=4, command=toggle_date_range_button).grid(row=2, column=3)
...
Label(root, text="Frequency:", font=("Arial", 11, "bold"), fg="#003366").grid(row=5, column=0, columnspan=4)
Radiobutton(root, text="daily", variable=interval_var, value='1d').grid(row=6, column=1, sticky='e')
Radiobutton(root, text="weekly", variable=interval_var, value='1wk').grid(row=6, column=2)
Radiobutton(root, text="monthly", variable=interval_var, value='1mo').grid(row=6, column=3, sticky='w')
...
max_min_checkbox.grid(row=9, column=0, columnspan=2, sticky='w')
